...
from attention.loop_CharModel import train_epoch, validate_epoch, test_epoch
...
if __name__ == "__main__":
    MODEL_LOC = "../05_models/"
    MODEL_TYPE = "CharWeighted"
    INVERSE = True
    BACKEND = "gloo"

    head_trial_number = str(datetime.datetime.now().strftime("%Y%m%d_%H%M"))
    save_loc = os.path.join(MODEL_LOC, f"_OOS_models_{MODEL_TYPE}_inverse={int(INVERSE)}_{head_trial_number}")
    os.makedirs(save_loc, exist_ok=True)

    all_start_test_months = [f"{d}-01-01" for d in np.arange(1983, 2022, 1)]
    for start_test_month in all_start_test_months[::-1]:
        LOCAL = True
        NUM_WORKERS = 1
        RESUME_TRIAL = ""

        timed_callout(f"Starting new model estimation: {start_test_month}")
        trial_number = datetime.datetime.now().strftime("%Y%m%d_%H%M")
        trial_name = MODEL_TYPE + f"_{start_test_month}_" + str(trial_number)
        ...
        # OPTIMIZER PARAMETER
        config["optim_params"] = dict(lr=0.001, amsgrad=True, weight_decay=0.01)
        # SCHEDULER PARAMETER
        config["scheduler_params"] = dict(max_lr=tune.grid_search([0.001, 0.005]), pct_start=0.2)

        # TUNE PARAMETER
        config["tune_params"] = dict(
            trial_stopper=True,
            trial_num_results=8,
            trial_grace_period=8,
            trial_tol=0.0,
        ...
        config = config_list.iloc[0].to_dict()

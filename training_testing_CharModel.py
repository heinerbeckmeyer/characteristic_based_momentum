...
# -- MODEL
from attention.data_loading import ModelDataset_CharModel, MyDistributedSampler
...
def test_func(config):
    ...
    # ---- DATASETS
    test_data = ModelDataset_CharModel(
        filename="../04_results/Momentum_DataSet_TargetOnly=False_std=ranks.pq",
        val_pct=val_pct,
        name="test",
    )
...
if __name__ == "__main__":
    ...
    LOCAL = True
    NUM_WORKERS = 1
    RESUME_TRIAL = "CharWeighted_posEnc=True___20230105_2023"
    ...
    if RESUME_TRIAL != "":
        ...
        trial_name = MODEL_TYPE + f"_posEnc=True___" + str(trial_number)
    ...
    # OPTIMIZER PARAMETER
    config["optim_params"] = dict(lr=0.01, amsgrad=True, weight_decay=0.01)
    # SCHEDULER PARAMETER
    config["scheduler_params"] = dict(max_lr=0.005, pct_start=0.2)
    # config["scheduler_params"] = dict(max_lr=tune.grid_search([0.1, 0.01, 0.001]), pct_start=0.2)

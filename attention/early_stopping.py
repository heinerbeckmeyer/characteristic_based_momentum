...
class ExperimentAndTrialPlateauStopper(Stopper):
    ...
    def __call__(self, trial_id: str, result: Dict):
        ...
        if "r2" in result.keys():
            self.r2[trial_id].append(result["r2"])
        if "epd" in result.keys():
            self.epd[trial_id].append(result["epd"])
        ...
        if self._trial_stopper:
            ...
            if (
                ...
                | (best > (100 * self.exp_top_mean))  # best score far from mean best scores
    ...
    def trial_logger(self, result, trial_id, best, no_improvement, iterations):
        with open(self._trial_logfile, "a+") as log:
            ...
            print(
                "   ".join("%.6f" % i for i in self.scores[trial_id][-self._num_results :]), file=log,
            )
            print(
                "Best score %.6f; no improvement for %d iterations." % (best, no_improvement), file=log,
            )
            if self.r2[trial_id]:
                print(f"R2", file=log)
                print(
                    "   ".join("%.6f" % i for i in self.r2[trial_id][-self._num_results :]), file=log,
                )
            if self.epd[trial_id]:
                print("EPD", file=log)
                print(
                    "   ".join("%.6f" % i for i in self.epd[trial_id][-self._num_results :]), file=log,
                )
            print("\n\n", file=log)
